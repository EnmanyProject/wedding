...
- **해결**:
  1. **elegant-loading.css 수정**:
     - `.global-loading-overlay.hidden`에 `pointer-events: none !important` 추가
     - `visibility: hidden` 추가로 완전히 숨김 처리
     - z-index: 9999 (모달보다 낮게 설정)
  2. **user-profile-modal.css 수정**:
     - animation 기반 opacity → transition 기반으로 변경
     - `.profile-modal`에 초기 `opacity: 0` 설정
     - `.profile-modal.active`에 명시적 `opacity: 1` 설정
     - `transition: opacity 0.3s ease-out` 추가
  3. **ui.js 개선**:
     - `aria-hidden` 속성 명시적 설정
     - 디버그 로깅 추가
...
**수정된 파일**:
- `public/styles/elegant-loading.css` (로딩 오버레이)
- `public/styles/user-profile-modal.css` (모달 opacity)
- `public/js/ui.js` (접근성 개선)
- `scripts/test-profile-modal.ts` (Playwright 테스트)
